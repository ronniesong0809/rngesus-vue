  ...
  methods: {
    async fetchPost() {
      getPost(this.$route.params.id).then(response => {
        const { data } = response;
        this.ruleForm.id = data.post.id;
        this.ruleForm.title = data.post.title;
        this.ruleForm.content = data.post.content;
        this.ruleForm.tags = data.tags.map(tag => tag.name);
    ...
    submitForm() {
      ...
      editPost(this.ruleForm).then(res => {
        if (res.code === 200) {
